...
// URL base da API Node/Express (server.js). Em produção, troque pelo domínio real.
const API_BASE_URL = "/api";

// Chave-mestra: enquanto false, o site usa dados mockados + localStorage.
// Agora que o backend Node/Express + SQLite está pronto, fica true.
const USE_API = true;
...
async function handleLoginSubmit(e) {
  ...
  try {
    ...
    if (result.success) {
      if (result.token) localStorage.setItem(STORAGE_KEYS.TOKEN, result.token);
...
async function handleRegisterSubmit(e) {
  ...
  try {
    const result = await registerUser({ ...user, senha });
    if (result.success) {
      if (result.token) localStorage.setItem(STORAGE_KEYS.TOKEN, result.token);
...
async function handlePlaceOrder() {
  ...
  const metodoPagamento = document.querySelector(".payment-tab.active")?.dataset.payment || "pix";
  const subtotal = cartSubtotal();
  const desconto = cartDiscount();
  const frete = getSelectedShippingCost();
  const total = subtotal - desconto + frete;

  const endereco = {
    cep: document.getElementById("chk-cep")?.value,
    rua: document.getElementById("chk-rua")?.value,
  ...
  const order = { itens: [...state.cart], subtotal, desconto, frete, total, metodoPagamento, endereco };

  try {
    const result = await createOrder(order);

    if (metodoPagamento === "pix" && result.pix) {
      // Mostra o código PIX (copia e cola) recebido do backend.
      const pixBox = document.querySelector("#payment-pix .pix-box p:nth-of-type(1)");
      if (pixBox) {
        pixBox.innerHTML = `<textarea readonly style="width:100%;min-height:70px;">${result.pix.codigo}</textarea>
          <button type="button" class="btn btn-outline" style="margin-top:8px;" onclick="navigator.clipboard.writeText('${result.pix.codigo}');showToast('Código PIX copiado!')">Copiar código</button>`;
      }
      showToast("Pedido criado! Pague o PIX gerado para confirmar.");
    } else {
    ...
    renderCartBadge();
    renderCartDrawer();
    if (!(metodoPagamento === "pix" && result.pix)) showView("home");
  } catch (err) {
    showToast(err.message || "Não foi possível confirmar o pedido.");
  }
}
...
async function handleAdminLogin(e) {
  ...
  try {
    ...
    showToast(err.message || "Usuário ou senha inválidos.");
  }
}
    ...
    <div class="modal-card">
      ...
      <form id="product-form">
        <div class="form-grid-2">
          ...
          <label>Imagem principal (URL) <input type="url" id="pf-imagem-1" value="${p && p.imagens[0] ? p.imagens[0] : ""}" placeholder="https://..."></label>
          <label>Imagem secundária 1 (URL) <input type="url" id="pf-imagem-2" value="${p && p.imagens[1] ? p.imagens[1] : ""}" placeholder="https://... (opcional)"></label>
          <label>Imagem secundária 2 (URL) <input type="url" id="pf-imagem-3" value="${p && p.imagens[2] ? p.imagens[2] : ""}" placeholder="https://... (opcional)"></label>
          <label>Imagem secundária 3 (URL) <input type="url" id="pf-imagem-4" value="${p && p.imagens[3] ? p.imagens[3] : ""}" placeholder="https://... (opcional)"></label>
        </div>
        <p class="field-note">A imagem principal aparece no card do produto. As secundárias aparecem ao passar o mouse (a 1ª) e como ângulos extras na página do produto. Só a principal é obrigatória.</p>
...
async function handleProductFormSubmit(e) {
  ...
  const imagens = [
    document.getElementById("pf-imagem-1").value.trim(),
    document.getElementById("pf-imagem-2").value.trim(),
    document.getElementById("pf-imagem-3").value.trim(),
    document.getElementById("pf-imagem-4").value.trim()
  ].filter(Boolean); // remove os campos deixados em branco
  ...
  const data = {
    ...
    imagens: imagens.length ? imagens : ["https://images.unsplash.com/photo-1521572163474-6864f9cf17ab?q=80&w=900&auto=format&fit=crop"],
  ...
  try {
    if (state.editingProductId) {
      ...
      state.products[idx] = result?.product ? result.product : { ...state.products[idx], ...data, id: state.editingProductId };
      showToast("Produto atualizado.");
    } else {
      const result = await createProduct(data);
      // O backend devolve { success, product }. Usamos o objeto "product"
      // completo (com o ID de verdade gerado no banco) em vez de tentar
      // remontar o produto localmente — assim ele fica 100% igual ao que
      // está salvo, e o ID bate certinho com o que o carrinho/favoritos usam.
    ...
    showToast(err.message || "Não foi possível salvar o produto.");
  }
}
